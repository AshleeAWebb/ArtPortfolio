...
const AboutMe = () => {
  ...
  return (
    <div>
      ...
      <h2 onClick={() => setBioOpen(!isBioOpen)}>
        Artist Bio {isBioOpen ? "▼" : "►"}
      </h2>
      {isBioOpen && (
        <p className="aboutMeInfo">Is the monster under the bed simply misunderstood? Do you have to be dead to be a ghost?
          ...
          She explores the view that not everything has to fit into a label.</p>
      )}
      <h2 onClick={() => setStatementOpen(!isStatementOpen)}>
        Artist Statement {isStatementOpen ? "▼" : "►"}
      </h2>
      {isStatementOpen && (
        <p className="aboutMeInfo">There is an attraction to breaking the rules. My art does just that by challenging socially imposed stereotypes and
          ...
          into the beauty of nonconformity.</p>
      )}
      <div>
